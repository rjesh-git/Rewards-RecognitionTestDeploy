...
namespace Microsoft.Teams.Apps.RewardAndRecognition.Cards
{
    ...
    public static class NominateCarouselCard
    {
        ...
        public static IEnumerable<Attachment> GetAwardsCard(IEnumerable<AwardEntity> awards, IStringLocalizer<Strings> localizer, TaskModuleResponseDetails details)
        {
            ...
            foreach (var award in awards)
            {
                AdaptiveCard carouselCard = new AdaptiveCard(new AdaptiveSchemaVersion(Constants.AdaptiveCardVersion))
                {
                    Body = new List<AdaptiveElement>
                    {
                        ...
                        new AdaptiveImage
                        {
                            Url = string.IsNullOrEmpty(award.AwardLink) ? default : new Uri(award.AwardLink),
