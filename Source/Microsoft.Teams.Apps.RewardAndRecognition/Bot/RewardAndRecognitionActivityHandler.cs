    ...
    public sealed class RewardAndRecognitionActivityHandler : TeamsActivityHandler
    {
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (valuesfromTaskModule.Command.ToUpperInvariant() == Constants.SaveNominatedDetailsAction)
                {
                    ...
                    var notificationCard = await turnContext.SendActivityAsync(MessageFactory.Attachment(EndorseCard.GetEndorseCard(this.localizer, valuesfromTaskModule)));
        ...
        protected override async Task<TaskModuleResponse> OnTeamsTaskModuleFetchAsync(ITurnContext<IInvokeActivity> turnContext, TaskModuleRequest taskModuleRequest, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var cycleStatus = await this.rewardCycleStorageProvider.GetActiveRewardCycleAsync(teamsDetails.Id);
                if ((cycleStatus == null || cycleStatus.RewardCycleState == (int)RewardCycleState.InActive) && valuesforTaskModule.Command != Constants.ConfigureAdminAction)
                {
            ...
            try
            {
                ...
                switch (valuesfromTaskModule.Command.ToUpperInvariant())
                {
                    ...
                    case Constants.NominateAction:
                        ...
                        await turnContext.SendActivityAsync(MessageFactory.Carousel(NominateCarouselCard.GetAwardsCard(awardsList, this.localizer, valuesfromTaskModule)));
                        break;

                    case Constants.SaveNominatedDetailsAction:
                        turnContext.Activity.Conversation.Id = valuesfromTaskModule.TeamId;
                        var result = await turnContext.SendActivityAsync(MessageFactory.Attachment(EndorseCard.GetEndorseCard(this.localizer, valuesfromTaskModule)));
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                return new MessagingExtensionResponse
                {
                    ComposeExtension = await SearchHelper.GetSearchResultAsync(searchQuery, teamsChannelData.Channel.Id, messageExtensionQuery.QueryOptions.Count, messageExtensionQuery.QueryOptions.Skip, this.nominateDetailSearchService, this.localizer),
                };
            }
            catch (Exception ex)
            {
