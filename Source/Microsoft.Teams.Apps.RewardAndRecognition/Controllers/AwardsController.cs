...
namespace Microsoft.Teams.Apps.RewardAndRecognition.Controllers
{
    ...
    public class AwardsController : BaseRewardAndRecognitionController
    {
        ...
        /// <returns>Returns true for successful operation.</returns>
        [HttpPost("DeleteAwards")]
        public async Task<IActionResult> DeleteAsync([FromBody] IList<string> awardIds)
        {
            try
            {
                if (awardIds == null)
                ...
                return this.Ok(await this.storageProvider.DeleteAwardsAsync(awardIds));
            }
            catch (Exception ex)
            {
